...
{
  /**
   * @brief A smart pointer that holds a pointer to a value, but does not own
   * it.
   * @details observer_ptr is a non-owning pointer, or observer. The observer
   * stores a pointer to a second object, known as the watched object. An
   * observer_ptr may also have no watched object.
   *
   * An observer is not responsible in any way for the watched object; there is
   * no inherent relationship between an observer and the object it watches.
   *
   * It is intended as a near drop-in replacement for raw pointer types, with
   * the advantage that, as a vocabulary type, it indicates its intended use
   * without need for detailed analysis by code readers.
   *
   * @remarks This is a reimplementation of the LF TSv2
   * <tt>std::observer_ptr</tt> type.
  ...
  class observer_ptr
  {
   public:
    ...
    /**
     * @brief The value type. Equivalent to the element type without
     * cv-qualifiers.
     */
     ...
     * @tparam U Underlying type of the unique pointer. Must be convertible to
     * <tt>T*</tt>.
    ...
    /**
     * @brief Returns <tt>true</tt> if the watched pointer is not
     * <tt>nullptr</tt>.
     */
     ...
     * @return The watched pointer.
     */
    [[nodiscard]] explicit operator pointer() const noexcept
    {
      return this->ptr_;
    }

    /**
     * @brief Returns the watched pointer and resets the watched pointer to
     * <tt>nullptr</tt>.
     * @return The watched pointer.
     */
    ...
    /**
     * @brief Resets the watched pointer to <tt>nullptr</tt> or the new watched
     * pointer.
...
namespace std
{
   ...
   * @relates leng::observer_ptr
   */
  template <typename T>
  void swap(leng::observer_ptr<T>& lhs, leng::observer_ptr<T>& rhs) noexcept
  {
    lhs.swap(rhs);
  }
   ...
   */
  template <typename T>
  struct hash<leng::observer_ptr<T>>
  {
    std::size_t operator()(leng::observer_ptr<T> const& ptr) const noexcept
    {
